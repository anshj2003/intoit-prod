...
def block_user():
    ...
    # Get blocker_id from email
    cursor.execute("SELECT id FROM users WHERE email = %s", (blocker_email,))
    blocker_data = cursor.fetchone()
    ...
    blocker_id = blocker_data[0]

    # Check if the user is already blocked
    cursor.execute("SELECT id FROM blocked_users WHERE blocker_id = %s AND blocked_id = %s", (blocker_id, blocked_id))
    blocked_data = cursor.fetchone()
...
def get_blocked_users():
    blocker_email = request.args.get('identifier')

    conn = get_db_connection()
    cursor = conn.cursor(cursor_factory=RealDictCursor)

    # Get the blocker_id using the identifier (email)
    cursor.execute("SELECT id FROM users WHERE email = %s", (blocker_email,))
    blocker_data = cursor.fetchone()
    ...
    cursor.execute("SELECT blocked_id FROM blocked_users WHERE blocker_id = %s", (blocker_id,))
    blocked_ids = cursor.fetchall()

    cursor.close()
    conn.close()

    return jsonify([blocked['blocked_id'] for blocked in blocked_ids])
